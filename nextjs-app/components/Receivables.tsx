...
export default function Receivables({ refreshTrigger, onPaymentRecorded }: { refreshTrigger: number, onPaymentRecorded: () => void }) {
    ...
    const fetchReceivables = async () => {
        try {
            ...
            const raw = Array.isArray(data) ? data : (data as { receivables?: unknown[] })?.receivables || [];
            const receivablesArray: ReceivableItem[] = raw.map((item) => {
                const i = item as Record<string, unknown>;
                return { ...i, pending_amount: ((i.total_amount as number) || 0) - ((i.paid_amount as number) || 0) } as ReceivableItem;
            });
